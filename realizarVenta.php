    ...
    <body>
        ...
        <section>
            <div style='text-align: center;'>
                <h1 style='color: blueviolet;'>Promedio Estudiantes</h1>    
            ...
            <div>
                <form method='post' action='realizarVenta.php'>
                    <center><table border='0'style='background-color: white;'>
                            ...
                            <tr>
                                <td colspan="2"><input type="submit" name="btnCom" value="Comprar"> </td>
                            </tr>
                            <tr>
                                <td >                               
                                    <?php
                                    if (!empty($_POST["txtNom"])) {
                                        for ($i = 0; $i < count($nombreProducto); $i++) {
                                            if ($_POST["txtNom"] == $nombreProducto[$i]) {
                                                $encontro = true;
                                                $pos = $i;
                                            }//Cierre if
                                            if ($_POST["txtCan"] == $stock[$i]) {
                                                $encontro = true;
                                                $pos = $i;
                                            }
                                        }//Cierre for
                                    } else {
                                        $mensaje = "Producto no esta registrado";
                                    }
                                    if (isset($_POST['btnCom'])) {
                                        if ($stock [$pos]>= $_POST["txtCan"]) {
                                            $totalVenta = $precioUnd [$pos]* $_POST["txtCan"];
                                            echo "<p>Venta realizada:</p>
                                                <ul>
                                              <li>Código: {$codigoProducto[$pos]}</li>
                                            <li>Nombre: {$nombreProducto[$pos]}</li>
                                            <li>Cantidad: $precioUnd[$pos]</li>
                                            <li>Total: $totalVenta</li>
                                            </ul>";
                                        } else {
                                            echo "El stock es insuficiente";
                                        }
                                    }
                                    ?>                                   
                                </td>
